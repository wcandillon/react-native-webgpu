...
// with `byteLength` as the size, WITHOUT validating those values against the
// real size of the backing ArrayBuffer. A spoofed object (or a view over a
// detached / resized buffer) can therefore produce an out-of-bounds pointer or
...
describe("ArrayBuffer bounds", () => {
  ...
  it("writes a typed-array view that has a non-zero byteOffset", async () => {
  ...
  it("respects dataOffset/size element arguments of writeBuffer", async () => {
    const result = await client.eval(({ device }) => {
  ...
  it("accepts a raw ArrayBuffer as the data source", async () => {
    const result = await client.eval(({ device }) => {
  ...
  // size. Before that fix they may corrupt memory / crash instead of throwing.

  it("rejects a spoofed BufferSource whose byteLength exceeds the buffer", async () => {
    await expect(
      client.eval(({ device }) => {
  ...
  it("rejects a spoofed BufferSource whose byteOffset is past the end", async () => {
    await expect(
      client.eval(({ device }) => {
        const realBuffer = new ArrayBuffer(4);
        const spoofed = {
  ...
  it("rejects a BufferSource with a negative byteOffset", async () => {
    await expect(
      client.eval(({ device }) => {
