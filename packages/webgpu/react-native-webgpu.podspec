...
Pod::Spec.new do |s|
  ...
  # This podspec is CocoaPods' default linking path for every supported React
  # Native version. On React Native 0.87+, an app that has opted into native
  # SwiftPM autolinking instead consumes packages/webgpu/Package.swift
  # directly (see that file's header comment) - CocoaPods and this podspec
  # are not involved in that path at all, and RN's own codegen tooling reads
  # this podspec's metadata regardless of which path links the module.
  s.source_files = [
