...
struct GPUPipelineLayoutDescriptor {
  std::vector<std::variant<std::nullptr_t, std::shared_ptr<GPUBindGroupLayout>>>
      bindGroupLayouts; // Iterable< | GPUBindGroupLayout | null | undefined >
  std::optional<double> immediateSize; // GPUSize32
  ...
  fromJSI(jsi::Runtime &runtime, const jsi::Value &arg, bool outOfBounds) {
    ...
    if (!outOfBounds && arg.isObject()) {
      ...
      if (value.hasProperty(runtime, "bindGroupLayouts")) {
        ...
        result->bindGroupLayouts = JSIConverter<std::vector<std::variant<
            std::nullptr_t, std::shared_ptr<GPUBindGroupLayout>>>>::
            fromJSI(runtime, prop, false);
      }
      if (value.hasProperty(runtime, "immediateSize")) {
        auto prop = value.getProperty(runtime, "immediateSize");
