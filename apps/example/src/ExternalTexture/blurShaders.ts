// Two shaders that together feed the ambient effect with a true large-kernel
// gaussian blur, instead of the spiral approximation in the main fragment.
//
// 1. PREPASS_SHADER. Renders the external (YUV) texture into a regular
...
//
// 2. BLUR_SHADER. The tile-based separable gaussian compute shader from the
...
@fragment
fn fs_main(in: VsOut) -> @location(0) vec4f {
  // Cover projection. Same math as computeUvScale(0u) in shader.ts so the
  // blurred backdrop lines up with the canvas's cover-mode framing.
