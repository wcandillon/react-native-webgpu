...
async function getAsset(input: string | number): Promise<string> {
  if (typeof input === "string") {
    ...
    if (
      ...
      input.startsWith(NativeModules.BlobModule?.BLOB_URI_SCHEME)
    ) {
  ...
  const asset = { localUri: undefined, uri: "Test" };
...
export function polyfills() {
  ...
  if (Platform.OS !== "web") {
    try {
      ...
      const { createFromParts } = BlobManager;
      BlobManager.createFromParts = function (
  ...
  const extractUrlBase = THREE.LoaderUtils.extractUrlBase.bind(
    THREE.LoaderUtils
  );
  ...
  THREE.TextureLoader.prototype.load = function load(
    ...
    if (this.path && typeof url === "string") {
      url = this.path + url;
    }
