---
title: Vision Camera
description: Real-time GPU effects on live camera frames - Native Media capstone.
---
...
<Callout type="info" title="Capstone">
This is the **application** chapter of [Native Media](/docs/native-media/native-textures). Read [External Textures](/docs/native-media/external-textures) and [Video Player](/docs/native-media/video-player) first if you have not imported native frames yet.
</Callout>
...
React Native WebGPU extends that snippet with **`rotation`** and **`mirrored`** on `importExternalTexture` so Dawn uprights the sensor image before your shader runs - see [External Textures](/docs/native-media/external-textures).
...
Create the device on the **main thread** and pass it into the worklet - `device.lost` only fires on the main JS runtime ([Reanimated](/docs/integrations/reanimated)).

See [Native Textures](/docs/native-media/native-textures) for feature detection on emulators vs physical devices.
...
- [External Textures](/docs/native-media/external-textures) - `importExternalTexture` lifecycle on React Native
- [Reanimated](/docs/integrations/reanimated) - worklet runtimes and `installWebGPU()`
...
On Android you must apply YUV→RGB in WGSL when hardware conversion is unavailable. See [External Textures](/docs/native-media/external-textures) for platform notes.
</Callout>
