...
TF.js requests its own `GPUDevice` via global `navigator.gpu` ([Device](/docs/rendering/device)). You do not pass a device from `useDevice()` unless you build a custom integration.
...
| Topic | Detail |
| ----- | ------ |
| **No present()** | Rendering chapters ([Canvas](/docs/rendering/canvas), [Present](/docs/rendering/present)) do not apply |
...
| **Combined apps** | Use TF.js for inference and WebGPU canvas for overlays ([Native Media](/docs/native-media/native-textures)) |

## When to use which stack

| Use case | Stack |
| -------- | ----- |
| Custom graphics | Canvas + WGSL ([Rendering](/docs/rendering/canvas)) |
