...
Both packages follow Chrome/Skia milestones: react-native-webgpu pins the Dawn revision from the Skia milestone's `DEPS`, and react-native-skia ships that milestone's binaries as `react-native-skia-graphite-*` `<milestone>.0.0`. Pick one row and install versions from it.

| Milestone | react-native-webgpu | Dawn release tag | `@shopify/react-native-skia` (Graphite `@next`) |
| --- | --- | --- | --- |
| m154 | >= 0.10.0 | `dawn-chrome-m154` | 2.12.0-next.1 (Graphite `154.0.0`) |
...
- **Idempotent.** Calling `importDevice(Skia.getNativeDevice())` again returns the same `GPUDevice` object on that runtime, so components and hooks can call it freely instead of threading a single instance around.
