...
On iOS, Canvas uses a `CAMetalLayer`. On Android it picks one of three backing views: a `SurfaceView` for an opaque canvas, and for a transparent one a `HardwareBufferView` on Android 10 (API 29) and above or a `TextureView` below that. The [Canvas API page](/api/canvas#android-rendering-options) compares the three and explains how to pick one explicitly.
