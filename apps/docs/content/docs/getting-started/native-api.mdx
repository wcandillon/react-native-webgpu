...
`importDevice()` is idempotent: calling it again with the same pointer on the same runtime returns the same `GPUDevice` object, for as long as that object is alive. Its `queue`, event listeners, `lost` promise and any of your own caches keyed by device (a `WeakMap`, a three.js renderer) therefore all refer to one device. Each JS runtime (main, Reanimated UI, worklet runtimes) has its own wrapper for a given pointer, since JS objects cannot cross runtimes.
