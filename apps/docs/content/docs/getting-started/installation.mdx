...
2. From your app's `ios` directory, run `npx react-native spm add` once to inject the Swift packages into the `.xcodeproj`, then build the project (not a workspace) as usual. React Native's `Sync SPM Autolinking` build phase picks `react-native-webgpu` up as a self-managed package and links its `ReactNativeWebGPU` product.
3. Do not run `pod install` in that app: it would re-integrate CocoaPods and break the Swift package graph.

The manifest consumes React Native's headers as Swift package products (`ReactHeaders`, `ReactNativeHeaders`, `ReactNativeDependenciesHeaders`, plus the app's generated `ReactAppHeaders` for the `RNWgpuViewSpec` codegen output), so it works with either the static-library or `use_frameworks!` layout. If a Graphite build of `@shopify/react-native-skia` is installed alongside, the manifest also checks that both packages link the same Dawn release and fails package resolution on a mismatch (see [React Native Skia](/docs/integrations/react-native-skia)). It follows the same integration model as `@shopify/react-native-skia`'s own `Package.swift`, so the two libraries link the same way.
