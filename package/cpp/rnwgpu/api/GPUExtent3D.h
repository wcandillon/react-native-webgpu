  ...
  fromJSI(jsi::Runtime &runtime, const jsi::Value &arg, bool outOfBounds) {
    ...
    if (!outOfBounds && arg.isObject()) {
      if (arg.getObject(runtime).isArray(runtime)) {
        ...
        if (size == 0) {
          throw std::runtime_error("Expected an array of size >1 for GPUTExtent3D");
        }
        if (size > 0) {
          result->_instance.width = array.getValueAtIndex(runtime, 0).asNumber();
        }
        if (size > 1) {
          result->_instance.height = array.getValueAtIndex(runtime, 1).asNumber();
        }
        if (size > 2) {
          result->_instance.depthOrArrayLayers = array.getValueAtIndex(runtime, 2).asNumber();
        }
      } else {
        auto object = arg.getObject(runtime);
        if (object.hasProperty(runtime, "width")) {
          result->_instance.width = object.getProperty(runtime, "width").asNumber();
        } else {
          throw std::runtime_error("Property GPUTExtent3D::width is required");
        }
        if (object.hasProperty(runtime, "height")) {
          result->_instance.height = object.getProperty(runtime, "height").asNumber();
        }
        if (object.hasProperty(runtime, "depthOrArrayLayers")) {
          result->_instance.depthOrArrayLayers = object.getProperty(runtime, "depthOrArrayLayers").asNumber();
        }
      }
    } else {
