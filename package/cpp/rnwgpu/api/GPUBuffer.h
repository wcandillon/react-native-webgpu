...
public:
    explicit GPUBuffer(wgpu::Buffer instance, std::shared_ptr<AsyncRunner> async, std::string label) : HybridObject("GPUBuffer"), _instance(instance), _async(async), _label(label) {}

public:
  std::string getBrand() { return _name; }


  std::future<void> mapAsync(uint64_t mode, std::optional<size_t> offset, std::optional<size_t> size);
std::shared_ptr<ArrayBuffer> getMappedRange(std::optional<size_t> offset, std::optional<size_t> size);
void unmap();
void destroy();
