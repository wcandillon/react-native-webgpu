...
public:
  void addSurface(const int contextId, void* surface, float width, float height) {
    _registry[contextId] = std::make_shared<Canvas>(reinterpret_cast<uint64_t>(surface), width, height);
  }
